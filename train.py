...
def focal_loss(y_pred, y_true, alpha=1, gamma=3, epsilon=1e-7):
    ...
    ce_loss = torch.nn.functional.cross_entropy(
        y_pred, y_true, reduction="none", ignore_index=10
...
class CSA(Dataset):
    ...
    def __getitem__(self, key):
        ...
        self.aug = self.taug if self.is_train else self.vaug
...
def train(
    ...
    for epoch in range(max_epochs):
        ...
        for _, sample_batched in enumerate(train_loader):
            ...
            image = mixup_batch(image, random.random() * 0.25).to(device)

            optimizer.zero_grad()

            output = model(image.to(device))

            total_loss = f1_loss(output, gt.to(device)) + focal_loss(
                output, gt.to(device)
            )
        ...
        total_score = report["weighted avg"]["f1-score"] * report["weighted avg"]["recall"]
        print(f" Validation Total Score: {total_score}")
        print(report["weighted avg"])
        
        if best_val_ < total_score:
            best_val_ = total_score
            step_donw = True
        ...
        if (epoch - best_epoch) > patience:
            print("Fnishing training, best validation acc %f", best_val_)
            break
...
def main():
    ...
    train(f"{model_type}_model.pth", device, model, nb_classes, train_loader, val_loader, max_epochs)
